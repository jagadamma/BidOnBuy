...
export const searchAndRefine = async (
  location,
  search,
  page = 1,
  mainCategory = "",
  ...
  const response = await axios.get(
    `/api/search/?location=${location}&search=${search}&category=${mainCategory}&subCategory=${subCategory}&year=${year}&sort=${sort}&page=${page}&limit=${limit}`,
    config
  );
  return response;
};
